...
public class IndexController {
    ...
    public ForumResult getNewUserInfo() {
        if (easyRedis.hasKey("newUserInfo")){
            Object newUserInfo = easyRedis.get("newUserInfo");
            log.info("从Redis中获取key："+"newUserInfo"+"-----value:"+newUserInfo.toString());
            return new ForumResult(200,"查询成功",newUserInfo);
        }
        ...
        easyRedis.set("newUserInfo",userInfoVOList,60*30);
        log.info("从Redis中添加key："+"newUserInfo"+"-----value:"+userInfoVOList.toString()+"有效时间为：3600");
     ...
     * 获取积分排行前几位的用户信息
     * @return 通用返回结果
     * redisKey:TopRateUserInfo
     */
    @RequestMapping("/getTopRateUserInfo")
    @ResponseBody
    public ForumResult getTopRateUserInfo(){

        if (easyRedis.hasKey("topRateUserInfo")){
            Object topRateUserInfo = easyRedis.get("topRateUserInfo");
            log.info("从Redis中获取key："+"topRateUserInfo"+"-----value:"+topRateUserInfo.toString());
            return new ForumResult(200,"查询成功",topRateUserInfo);
        }
        ...
        easyRedis.set("topRateUserInfo",res,60*30);
        log.info("从Redis中添加key："+"TopRateUserInfo"+"-----value:"+res.toString()+"有效时间为：3600");
    ...
    public ForumResult getQuestionInfoByViewCount() {
        ...
        if (easyRedis.hasKey("RecommendQuestion")){
            Object RecommendQuestion = easyRedis.get("RecommendQuestion");
            log.info("从Redis中获取key："+"topRateUserInfo"+"-----value:"+RecommendQuestion.toString());
            return new ForumResult(200,"查询成功",RecommendQuestion);
        }
        try {
            ...
            easyRedis.set("RecommendQuestion",res,60*30);
            log.info("从Redis中添加key："+"RecommendQuestion"+"-----value:"+res.toString()+"有效时间为：3600");
            return new ForumResult(200, "查询成功", res);
        } catch (Exception e) {
            e.printStackTrace();
